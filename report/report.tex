...
\begin{abstract}
We study the task of generating short, playable 2D platformer screens in the
...
(\,$\approx$0\% BFS reachability). We show that a lightweight
...
Our answer is yes, and the load-bearing piece is not the model --- it
...
addition to gameplay role. Since PCG should be theme-agnostic, we
...
chunks are mostly sky. We drop any chunk with $<$10\% non-air tiles
...
$<$1\% combined. This is load-bearing for training choices (we use
\emph{sqrt}-inverse class weights, not inverse-frequency --- see
Section~\ref{sec:models}) and for how we read the evaluation numbers
(tile-level distribution metrics will be dominated by the common
categories).

% ---------------------------------------------------------------------------
\section{Related Work}

The problem we address has a small but well-developed literature.
...
purely generative approaches --- a framing we adopt directly in our
...
objective. Our approach differs in two ways: (i) we use a conditional
VAE \citep{kingma2014auto,sohn2015learning} rather than a GAN for
tractability on a small dataset, and (ii) we make the playability
...
Our bigram classical baseline builds on the long history of grammar-
and transition-based PCG (\citet{smith2010tanagra} among others); our
naive baseline is intentionally minimal, following the course's
insistence on a \emph{truly} uninformative control.

Finally, the \emph{post-hoc repair} approach we ultimately rely on
...
\citet{earle2021illuminating}, though we apply it as a deterministic
...
Our evaluation treats generation as a constrained design problem with
...
We run a physics-aware breadth-first search from every valid standing
...
browser; we verified this ourselves by repeatedly failing ``playable''
...
We compare generated and training tile-category histograms:
\[
...
cross-model comparison meaningful. We pre-compute the train-set
...
diverse. We log this to catch the opposite failure mode: a model that
...
truth per position --- any position has many legitimate completions.
Our closest analogue to a confusion matrix is
...
\begin{enumerate}
    ...
    score each chunk by a hand-designed structural difficulty (gaps,
    hazards, height variance) and tertile-bin into easy / medium /
    hard. \emph{Rationale:} gives the VAE a conditioning signal we
...
With $\sim$1.5k training chunks on CPU, we could not afford a grid
search. We chose three configurations informed by failure modes
observed on the train/val split:
\begin{itemize}
    \item Inverse-frequency class weights collapsed the VAE to near-
    uniform output (rare hazards over-represented). We switched to
    \emph{sqrt}-inverse weights
    $w_c \propto 1/\sqrt{\operatorname{freq}(c)}$, capped at 1.5, which
    recovers a realistic histogram.
    \item KL weight $\beta = 0.5$ drove posterior collapse; we use
    $\beta = 0.1$.
...
burden of a neural model. Key implementation note: we initially tried
...
(collapsed: empty/solid/hazard) on the full 20$\times$40 grid. We use
sqrt-inverse class weights on the cross-entropy reconstruction loss,
$\beta=0.1$ KL weight, and train for 150 epochs on CPU. At inference we
...
is the centerpiece of our ablation and the subject of Section~\ref{sec:experiment}.
...
\begin{itemize}
    ...
    both --- a fair side-effect of the problem framing (we generate
...
\begin{figure}[h]
\centering
\includegraphics[width=0.95\linewidth]{../experiments/results/repair_ablation.png}
...
We picked five specific mispredictions spanning all five (model,
...
\begin{figure}[h]
\centering
\includegraphics[width=0.9\linewidth]{figures/error_cases.png}
...
That is our top priority under ``Future Work.''
...
post-pass?} We hold everything else constant and vary only the
presence of \texttt{scripts.repair.repair}. For each of six cells
(real@test, naive@50, bigram@50, vae@easy, vae@medium, vae@hard) we
...
--- higher than our conditional VAE does at any difficulty. The
...
\begin{itemize}
    ...
    full pipeline on that would let us compare naive/bigram/VAE at a
    ...
    we currently stitch 20$\times$40 chunks by copying the rightmost
    ...
    (and remove the stitching-discontinuity failure modes we see in
    endless mode today).
    \item \textbf{Human playtesting.} BFS playability is a proxy for
    ``fun.'' A paired user study (\emph{which level feels more fun?})
    would tell us whether VAE-diversity or bigram-consistency matters
...
\begin{enumerate}
    ...
    would need a much larger, licensable content dataset --- raising
    IP questions (see Ethics).
...
\paragraph{Honest assessment.} This is a useful infrastructure
...
rendering generated content is permissible under that license. We do
not redistribute the SuperTux assets themselves in our repository;
...
recognizable as SuperTux-adjacent. We explicitly do not claim
...
of difficulty on different runs. We disclose this in the UI (``levels
are generated per run''), but a strict competitive mode would need
a shared-seed option for fairness. We do not ship such a mode today.
...
endless-mode best). No PII beyond the self-chosen username; guest
